...
export default function NewPanel({ data, user, ticket, role, authorizations }) {
  // Le fichier STL ne s'ouvre pas.
  // La demande n'est pas claire, que dois-je faire ?
  // J'ai validé une impression alors que j'aurais dû la refuser.
  const faqs = [
    {
              ...
              <div className="block mt-5">
                <div className="sm:hidden">
                  <div className="px-4 sm:px-6">
                    <h2 className="text-gray-500 text-xs font-medium uppercase tracking-wide">Ticket</h2>
                  </div>
                </div>
                        ...
                        <tr key={project.id} onClick={() => router.push("/panel/" + project.id)} className="hover:bg-gray-50 cursor-pointer">
                          <td className="px-6 py-3 max-w-0 w-full whitespace-nowrap text-sm font-medium text-gray-900">
                            <div className="flex items-center space-x-3 lg:pl-2">
                              {getColor(project.step, project.waitingAnswer) == 10 ? (
                                ...
                                <div
                                  className={classNames(
                                    getColor(project.step, project.waitingAnswer) == 0
                                      ? "bg-indigo-200"
                                      : getColor(project.step, project.waitingAnswer) == 1
                                      ? "bg-amber-200"
                                      : getColor(project.step, project.waitingAnswer) == 2
                                      ? "bg-emerald-300"
                                      : getColor(project.step, project.waitingAnswer) == 3
                                      ? "bg-emerald-500"
                                      : getColor(project.step, project.waitingAnswer) == 4
                                      ? "bg-red-500"
                                      : "",
                              ...
                              <a className="truncate hover:text-gray-600">
                                <span>
                                  <Link href={`/panel/${project.id}`}>{"#" + setZero(project.id)}</Link>{" "}
                                </span>
                              </a>
                            </div>
                          </td>
