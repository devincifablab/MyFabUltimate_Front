...
const colors = {
  "PIX 1": "text-red-700 bg-red-200",
  "PIX 2": "text-yellow-700 bg-yellow-200",
...
export default function TablesAdmin({tickets, isDone}) {
  ...
  var ticket;
  if(isDone){
    ticket = tickets.filter(r => r.isDone == false);
  } else {
    ticket = tickets;
  }
  return (
          ...
          {ticket.map((r) => {
            return (
              <tr className="border-b border-gray-200">
                ...
                <td className="p-3">
                  <p className="font-medium">{r.user.name+(" "+r.user.surname).toString().toUpperCase()}</p>
                  <p className="text-gray-500">ESILV A2</p>
                </td>
                <td className="p-3 text-gray-500 truncate">{r.user.email}</td>
                <td className="p-3 text-center">
                  <div
                    className={`font-semibold inline-flex px-2 py-1 leading-4 text-xs rounded-full ${colors[r.type.toString()]}`}
                  >
                    {r.type}
                  </div>
                </td>
